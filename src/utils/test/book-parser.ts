import { BookParser } from 'utils/parsers/base'
import { FB2Parser } from 'utils/parsers/fb2'

const firstName = 'Тестовый'
const lastName = 'Автор'
const bookTitle = 'Тестовое название'
const bookImage =
  ''
const contentType = 'image/png'

describe('FB2Parser', () => {
  let parser: BookParser

  it('good data', async () => {
    const bookData = `<?xml version="1.0" encoding="UTF-8"?>
    <FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0" xmlns:l="http://www.w3.org/1999/xlink">
    <description>
      <title-info>
        <author>
          <first-name>${firstName}</first-name>
          <last-name>${lastName}</last-name>
        </author>
        <book-title>${bookTitle}</book-title>
        <coverpage>
          <image l:href="#cover.jpg"/>
        </coverpage>
      </title-info>
    </description>
    <binary id="cover.jpg" content-type="${contentType}">${bookImage}</binary>
    <binary id="test.jpg" content-type="${contentType}">123</binary>
    </FictionBook>`

    parser = new FB2Parser(new Blob([bookData]), '')

    await parser.parse()

    expect(parser.author).toEqual(`${firstName} ${lastName}`)
    expect(parser.title).toEqual(bookTitle)
    expect(parser.cover).toEqual({
      data: bookImage,
      fileName: 'testovyy_avtor_testovoe_nazvanie.jpg',
      type: contentType,
    })
  })

  describe('bad data', () => {
    it('should parse empty book when coverpage is not exist', async () => {
      const badBookData = `<?xml version="1.0" encoding="UTF-8"?>
        <FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0" xmlns:l="http://www.w3.org/1999/xlink">
        <description>
          <title-info>
            <author>
              <first-name>${firstName}</first-name>
              <last-name>${lastName}</last-name>
            </author>
            <book-title>${bookTitle}</book-title>
          </title-info>
        </description>
        </FictionBook>
        `
      parser = new FB2Parser(new Blob([badBookData]), '')

      await parser.parse()

      expect(parser.cover).toBeFalsy()
    })

    it('should parse empty book when coverpage is bad', async () => {
      const badBookData = `<?xml version="1.0" encoding="UTF-8"?>
        <FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0" xmlns:l="http://www.w3.org/1999/xlink">
        <description>
          <title-info>
            <author>
              <first-name>${firstName}</first-name>
              <last-name>${lastName}</last-name>
            </author>
            <book-title>${bookTitle}</book-title>
            <coverpage>
              <image l:href="#cover.jpg"/>
            </coverpage>
          </title-info>
        </description>
        </FictionBook>
        `
      parser = new FB2Parser(new Blob([badBookData]), '')

      await parser.parse()

      expect(parser.cover).toBeFalsy()
    })
  })
})
